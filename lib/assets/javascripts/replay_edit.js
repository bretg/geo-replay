...
function initialize() {
  ...
  if (app.appName) {
    setFieldValue("appName",app.appName);
  }
  if (app.appDescription) {
    setFieldValue("appDescription",app.appDescription);
  }
  if (app.author) {
	  setFieldValue("author",app.author)
  }
  if (app.tags) {
	  setFieldValue("tags",app.tags)
  }
  if (app.sources) {
    setFieldValue("sources",app.sources);
  }
  if (app.defaultLabelStyle) {
    setFieldValue("defaultLabelStyle",app.defaultLabelStyle);
  }
  if (app.mapType) {
    setFieldValue("mapType",app.mapType);
  }
  if (app.mapStyle) {
    setFieldValue("mapStyle",app.mapStyle);
  }
  if (app.animLength) {
    setFieldValue("animLength",app.animLength);
  }
  ...
  var centerPoint;
  if (app.mapCenter) {
	centerPoint=new google.maps.LatLng(app.mapCenter.lat, app.mapCenter.lng);
  } else {
  ...
  for(var p=0; p<numPaths; p++) {
	  ...
	  newPath.set("id",p);
	  newPath.set("lineName",app.paths[p].pathName);
	  newPath.set("animSymbol",app.paths[p].animSymol);
	  newPath.set("lineWidth",app.paths[p].pathWidth);
	  newPath.set("defaultTextOffsetX",app.paths[p].defaultTextOffsetX);
	  newPath.set("defaultTextOffsetY",app.paths[p].defaultTextOffsetY);
	  newPath.set("lineDefaultMarker",app.paths[p].defaultMarkerImage);
	  var numPoints=app.paths[p].points.length;
	  var llArray=new Array();
	  for(var n=0; n<numPoints; n++) {
		  newPoint=new google.maps.LatLng(app.paths[p].points[n].lat, app.paths[p].points[n].lng);
		  llArray.push(newPoint);
		  newPath.set("point"+n+"Label",app.paths[p].points[n].text);
		  newPath.set("point"+n+"Time",app.paths[p].points[n].time);
		  if (app.paths[p].points[n].date) {
			  newPath.set("point"+n+"Time",app.paths[p].points[n].date);
		  }
		  newPath.set("point"+n+"OffsetX",app.paths[p].points[n].textOffsetX);
		  newPath.set("point"+n+"OffsetY",app.paths[p].points[n].textOffsetY);
		  newPath.set("point"+n+"Details",app.paths[p].points[n].details);
		  newPath.set("point"+n+"Marker",app.paths[p].points[n].markerImage);
	  }
...
function setZoom() {
   app.mapZoom=map.getZoom();
}

function setCenter() {
   var lat=map.getCenter().lat();
   var lng=map.getCenter().lng();
   app.mapCenter=new Object();
   app.mapCenter.lat=lat;
   app.mapCenter.lng=lng;
}

function toCenter() {
   map.setZoom(app.mapZoom);
   map.setCenter(new google.maps.LatLng(app.mapCenter.lat,app.mapCenter.lng));
}
...
function getAppObj() {
	...
	app.labelStyles=appLabelStyles;
...
function setAppFieldValue(field) {
	var widget=document.getElementById(field);
	var command="app."+field+"=widget.value;";
	eval(command);
}
